...
const __suites = [];
function __defineSuite(name, fn) {
  __suites.push({ name, fn });
}
...
__defineSuite("test-harness", async function () {
      ...
      Object.assign(
        {
          ...
          donotresettimer: false,
          thresholdaction: "donothing",
          thresholdcount: "0",
          thresholdaddtime: "0",
          thresholdaddtimeunits: "Second",
          heartbeatinterval: "0",
  ...
  await (async function main() {
    ...
    require(path.join(__dirname, "..", "timer-events", "timer-events.js"))(RED);
    ...
    // -- T1: pause/resume accuracy with reporting off ("Never") ---------------
    {
      ...
      check(
        "T1a pause freezes at ~7000ms (was: full 10000ms)",
        p && near(p.remainingTime, 7000, 300),
        p && p.remainingTime,
      );
      ...
      check(
        "T1b remaining unchanged while paused",
        q && near(q.remainingTime, 7000, 300),
        q && q.remainingTime,
      );
      ...
      check(
        "T1c resume runs ~7s more, not 10s (was: full duration back)",
        near(ranFor, 7000, 500),
        ranFor,
      );
      await n.close(false);
    }
    ...
    // -- T2: query mid-run reports live remaining time ------------------------
    {
      ...
      const q = lastEvent(n, "query");
      check(
        "T2 query at t+4s reports ~6000ms (was: 10000ms)",
        q && near(q.remainingTime, 6000, 300),
        q && q.remainingTime,
      );
    ...
    // -- T3: adjusttime computes from true remaining, and never leaks into next run
    {
      ...
      check(
        "T3a adjust +5s at t+2s -> ~13000ms (was: 15000ms)",
        a && near(a.remainingTime, 13000, 300),
        a && a.remainingTime,
      );
      n.receive({ payload: "stop" });
      n.sent.length = 0;
      ...
      check(
        "T3b next run starts at original 10000ms (no adjustment leak)",
        s && near(s.remainingTime, 10000, 50),
        s && s.remainingTime,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    ...
    // -- T4: settime while paused updates the frozen snapshot ------------------
    {
      ...
      const q = lastEvent(n, "query");
      check(
        "T4 settime while paused -> frozen at exactly 4000ms",
        q && q.remainingTime === 4000,
        q && q.remainingTime,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    ...
    // -- T5: persistence writes the real target (reporting off) ----------------
    {
      ...
      check(
        "T5 persisted target reflects ~7000ms left (was: ~10000ms)",
        near(persistedRemaining, 7000, 400),
        Math.round(persistedRemaining),
      );
      n.receive({ payload: "stop" });
      await n.close(true);
    }
    ...
    // -- T6: cooldown query reports live cooldown remaining --------------------
    {
      ...
      const q = lastEvent(n, "query");
      check(
        "T6b cooldown query state",
        q && q.timerState === "cooldown",
        q && q.timerState,
      );
      check(
        "T6c cooldown remaining ~4000ms (was: full 6000ms)",
        q && near(q.remainingTime, 3900, 400),
        q && q.remainingTime,
      );
      n.receive({ payload: "stop" }); // cancel cooldown
      const st = lastEvent(n, "stopped");
      check(
        "T6d stop after cooldown reports remainingTime 0",
        st && st.remainingTime === 0,
        st && st.remainingTime,
      );
      await n.close(false);
    }
    ...
    // -- T7: regression - reporting ON still behaves (natural expiry timing) ---
    {
      ...
      check(
        "T7a expiry fires at ~4s with reporting on",
        near(Date.now() - t0, 4000, 500),
        Date.now() - t0,
      );
      const e = lastEvent(n, "expired");
      check(
        "T7b expired reports remainingTime 0",
        e && e.remainingTime === 0,
        e && e.remainingTime,
      );
      await n.close(false);
    ...
    // -- T8: regression - blocked restart while locked, envelope still sane ----
    {
      ...
      check(
        "T8 ignored restart carries live remaining ~8000ms",
        r && r.ignored === true && near(r.remainingTime, 8000, 300),
        r && r.remainingTime,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
...
__defineSuite("test-threshold-scope", async function () {
  ...
  function output2(node) {
    // messages that appeared on output 2 specifically
    return node.sent.map((a) => a[1]).filter(Boolean);
  }
  function lastEvent(node, type) {
    const e = events(node, type);
  ...
  await (async function main() {
    ...
    // -- U1: idle + disabled + Add Time threshold must NOT start the timer -----
    {
      const n = makeNode(RED, {
        thresholdaction: "addtime",
        thresholdcount: "2",
        thresholdaddtime: "10",
        donotresettimer: true,
      });
      ...
      check(
        "U1a timer stays idle (was: Add Time started it)",
        q.timerState === "stopped",
        q.timerState,
      );
      check(
        "U1b no timeadjusted fired from idle",
        events(n, "timeadjusted").length === 0,
        events(n, "timeadjusted").length,
      );
      check(
        "U1c blocked starts still counted",
        q.ignoredCount === 4,
        q.ignoredCount,
      );
      const blocked = events(n, "started").filter((e) => e.ignored === true);
      check(
        "U1d each block still observable on output 4",
        blocked.length === 4,
        blocked.length,
      );
      await n.close(false);
    ...
    // -- U2: idle + disabled + Stop threshold -> no phantom stop on output 2 ---
    {
      ...
      check(
        "U2 no phantom stopped on output 2",
        output2(n).length === 0,
        output2(n).length,
      );
      await n.close(false);
    ...
    // -- U3: cooldown + Restart threshold -> still never fires (unchanged) -----
    {
      ...
      check(
        "U3a no restart fired during cooldown",
        events(n, "restarted").length === 0,
        events(n, "restarted").length,
      );
      const q = (n.receive({ payload: "query" }), lastEvent(n, "query"));
    ...
    // -- U4: regression - lock gate + Add Time threshold still fires while running
    {
      ...
      const a = lastEvent(n, "timeadjusted");
      check(
        "U4a Add Time still fires for a running locked timer",
        !!a && a.source === "internal" && a.timeAdjusted === 5000,
        a && a.timeAdjusted,
      );
      check(
        "U4b remaining reflects the added time (~14500ms)",
        a && Math.abs(a.remainingTime - 14500) <= 400,
        a && a.remainingTime,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    ...
    // -- U5: regression - paused gate + Restart threshold fires, stays paused --
    {
      ...
      const r = events(n, "restarted").filter(
        (e) => e.ignored === false && e.source === "internal",
      );
      check(
        "U5a Restart threshold still fires from the paused gate",
        r.length === 1,
        r.length,
      );
      const q = (n.receive({ payload: "query" }), lastEvent(n, "query"));
      check(
        "U5b stays paused at full duration",
        q.timerState === "paused" && q.remainingTime === 10000,
        q.timerState + "/" + q.remainingTime,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    }
...
__defineSuite("test-restore", async function () {
  ...
  function output1(node) {
    return node.sent.map((a) => a[0]).filter(Boolean);
  }
  function lastEvent(node, type) {
    const e = events(node, type);
    return e.length ? e[e.length - 1] : null;
  ...
  await (async function main() {
    ...
    check(
      "V1a restore fires started, output 1, source internal",
      output1(n2).length === 1 &&
        started &&
        started.source === "internal" &&
        started.ignored === false,
      output1(n2).length + "/" + (started && started.source),
    );
    check(
      "V1b original timerDuration preserved: 10000 (was: remaining time)",
      started && started.timerDuration === 10000,
      started && started.timerDuration,
    );
    check(
      "V1c ignoredCount survives restore: 2 (was: reset to 0)",
    ...
    check(
      "V1d elapsedTime reflects run + downtime (~5000ms)",
      started && near(started.elapsedTime, 5000, 700),
      started && started.elapsedTime,
    );
    check(
      "V1e elapsed + remaining reconciles with duration",
      started &&
        near(
          started.elapsedTime + started.remainingTime,
          started.timerDuration,
          700,
        ),
      started && Math.round(started.elapsedTime + started.remainingTime),
    );
    check(
      "V1f remaining carried over minus downtime (~5000ms, was ~7000 pre-shutdown)",
      started &&
        near(started.remainingTime, 5000, 700) &&
        near(preQ.remainingTime, 7000, 700),
      started &&
        Math.round(started.remainingTime) +
          " (pre: " +
          Math.round(preQ.remainingTime) +
          ")",
    );

    // ---- V2: fresh config wins over persisted reporting settings -------------
    check(
      "V2 freshly-deployed reporting config wins (was: persisted 'every_second')",
      n2.reporting === "none",
      n2.reporting,
    );
    ...
    check(
      "V3 restored run expires after ~remaining (~5s), not full duration",
      near(ranFor, 5000, 800),
      ranFor,
    );
    const exp = lastEvent(n2, "expired");
    check(
      "V3b expired event still reports the original duration",
      exp && exp.timerDuration === 10000,
      exp && exp.timerDuration,
    );
    await n2.close(true);
    ...
    // ---- V4: regression - a normal start still resets run identity -----------
    {
      ...
      const s = lastEvent(n, "started");
      check(
        "V4 normal start resets ignoredCount and sets fresh duration",
        s &&
          s.ignoredCount === 0 &&
          s.lastIgnoredTime === null &&
          s.timerDuration === 10000 &&
          s.elapsedTime <= 50,
...
__defineSuite("test-validation", async function () {
  ...
  await (async function main() {
    ...
    // -- W1: msg.delay unconvertible ("5s") -> documented fallback -------------
    {
      ...
      const s = lastEvent(n, "started");
      check(
        "W1 delay '5s' falls back to configured 10000ms (was: NaN corruption)",
        s && near(s.remainingTime, 10000, 50) && s.timerDuration === 10000,
        s && s.remainingTime,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    ...
    // -- W2: msg.delay negative -> documented clamp to 0 ------------------------
    {
      ...
      const e = lastEvent(n, "expired");
      check(
        "W2 negative delay clamps to 0 and expires immediately (was: negative remaining)",
        !!e && e.timerDuration === 0,
        e && e.timerDuration,
      );
      await n.close(false);
    ...
    // -- W3: fractional delay still works (regression) -------------------------
    {
      ...
      const s = lastEvent(n, "started");
      check(
        "W3 fractional delay 2.5s -> 2500ms preserved",
        s && s.remainingTime === 2500,
        s && s.remainingTime,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    ...
    // -- W4: msg.delay empty string -> fallback, not zero -----------------------
    {
      ...
      const s = lastEvent(n, "started");
      check(
        "W4 empty-string delay falls back to configured (Number('') is 0 trap)",
        s && s.remainingTime === 10000,
        s && s.remainingTime,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    ...
    // -- W5: adjusttime missing / non-numeric -> ignored, timer unharmed --------
    {
      ...
      const a1 = lastEvent(n, "timeadjusted");
      check(
        "W5a missing adjusttime -> ignored:true, null attempted value",
        a1 && a1.ignored === true && a1.timeAdjusted === null,
        a1 && a1.ignored + "/" + a1.timeAdjusted,
      );
      n.receive({ payload: "adjusttime", adjusttime: "abc" }); // non-numeric
      const a2 = lastEvent(n, "timeadjusted");
      check(
        "W5b non-numeric adjusttime -> ignored:true, raw value attached",
        a2 && a2.ignored === true && a2.timeAdjusted === "abc",
        a2 && a2.timeAdjusted,
      );
      n.receive({ payload: "query" });
      const q = lastEvent(n, "query");
      check(
        "W5c remaining time unharmed (~9000ms, was: NaN)",
        q && near(q.remainingTime, 9000, 300),
        q && q.remainingTime,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    ...
    // -- W6: settime NaN passes-as-valid hole closed; valid settime unchanged ---
    {
      ...
      const t1 = lastEvent(n, "timeset");
      check(
        "W6a non-numeric settime -> ignored:true (NaN<=0 hole closed)",
        t1 && t1.ignored === true && t1.timeSet === "abc",
        t1 && t1.timeSet,
      );
      n.receive({ payload: "settime", settime: 4000 }); // regression: valid still works
      const t2 = lastEvent(n, "timeset");
      check(
        "W6b valid settime 4000 still applies",
        t2 && t2.ignored === false && near(t2.remainingTime, 4000, 50),
        t2 && t2.remainingTime,
      );
      n.receive({ payload: "settime", settime: -1 }); // regression: <=0 still rejected
      const t3 = lastEvent(n, "timeset");
      check(
        "W6c settime <= 0 still rejected, attempted value attached (ms default)",
        t3 && t3.ignored === true && t3.timeSet === -1,
        t3 && t3.timeSet,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    ...
    // -- W7: setduration NaN can no longer poison the next run ------------------
    {
      ...
      const d1 = lastEvent(n, "durationset");
      check(
        "W7a non-numeric setduration -> ignored:true",
        d1 && d1.ignored === true && d1.durationSet === "abc",
        d1 && d1.durationSet,
      );
      n.receive({ payload: "go" });
      const s1 = lastEvent(n, "started");
      check(
        "W7b next run unpoisoned: starts at configured 10000ms (was: NaN)",
        s1 && near(s1.remainingTime, 10000, 50),
        s1 && s1.remainingTime,
      );
      n.receive({ payload: "stop" });
      n.receive({
        payload: "setduration",
        setduration: 3,
        setdurationunits: "seconds",
      }); // regression
      n.receive({ payload: "go2" });
      const s2 = lastEvent(n, "started");
      check(
        "W7c valid setduration still applies to next run (3000ms)",
        s2 && near(s2.remainingTime, 3000, 50),
        s2 && s2.remainingTime,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    ...
    // -- W8: adjusttime 0 is processed, NOT flagged ignored (explicit decision) -
    {
      ...
      const a = lastEvent(n, "timeadjusted");
      check(
        "W8 adjusttime 0 accepted as successful no-op",
        a &&
          a.ignored === false &&
          a.timeAdjusted === 0 &&
          near(a.remainingTime, 9500, 300),
        a &&
          a.ignored + "/" + a.timeAdjusted + "/" + Math.round(a.remainingTime),
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    }
...
__defineSuite("test-redundant-stop", async function () {
  ...
  await (async function main() {
    ...
    // -- X1: stop on a never-started node -> ignored, output 4 only ------------
    {
      ...
      const st = lastEvent(n, "stopped");
      check(
        "X1a stop while stopped -> ignored:true on output 4",
        st && st.ignored === true && st.timerState === "stopped",
        st && st.ignored + "/" + st.timerState,
      );
      check(
        "X1b nothing on output 2 (was: phantom stop)",
        output2(n).length === 0,
        output2(n).length,
      );
      await n.close(false);
    ...
    // -- X2: stop after natural expiry -> ignored, state stays expired ---------
    {
      ...
      const st = lastEvent(n, "stopped");
      check(
        "X2a stop after expiry -> ignored:true, state stays 'expired' (was: flip to stopped)",
        st && st.ignored === true && st.timerState === "expired",
        st && st.timerState,
      );
      check(
        "X2b no phantom stop on output 2",
        output2(n).length === 0,
        output2(n).length,
      );
      check(
        "X2c zero counter touch",
        st && st.ignoredCount === 0 && st.lastIgnoredTime === null,
        st && st.ignoredCount + "/" + st.lastIgnoredTime,
      );
      n.receive({ payload: "stop" }); // second redundant stop, same treatment
      check(
        "X2d repeated stops all ignored",
        events(n, "stopped").every((e) => e.ignored === true) &&
          events(n, "stopped").length === 2,
        events(n, "stopped").length,
      );
      await n.close(false);
    }
    ...
    // -- X3: post-expiry stop no longer arms the _timerpass filter -------------
    {
      ...
      const s = lastEvent(n, "started");
      check(
        "X3 _timerpass msg starts normally after ignored stop (filter not armed)",
        s && s.ignored === false,
        s && s.timerEvent + "/" + s.ignored,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    }
    ...
    // -- X4: regression - genuine stop still arms _timerpass filter ------------
    {
      ...
      check(
        "X4a genuine stop fires on output 2",
        output2(n).length === 1 && output2(n)[0].timerEvent === "stopped",
        output2(n).length,
      );
      n.sent.length = 0;
      n.receive({ payload: "go", _timerpass: true }); // must die silently
      check(
        "X4b armed filter still swallows _timerpass msg (no output at all)",
        n.sent.length === 0,
        n.sent.length,
      );
      n.receive({ payload: "stop", _timerpass: true }); // stop variant also swallowed
      check(
        "X4c armed filter swallows _timerpass stop too (preserved edge)",
        n.sent.length === 0,
        n.sent.length,
      );
    ...
    // -- X5: regression - stop stays genuine for paused and cooldown -----------
    {
      ...
      n.receive({ payload: "stop" });
      check(
        "X5a stop while paused is genuine (output 2)",
        output2(n).length === 1 && lastEvent(n, "stopped").ignored === false,
        output2(n).length,
      );
      await n.close(false);
      ...
      check(
        "X5b stop during cooldown is genuine and cancels it",
        output2(c).length === 1 &&
          st.ignored === false &&
    ...
    // -- X6: disable harmonized; blocked idle starts still count ---------------
    {
      ...
      const d = lastEvent(n, "disabled");
      check(
        "X6a redundant disable no longer increments (was: lone oddball)",
        d &&
          d.ignored === true &&
          d.ignoredCount === 0 &&
          d.lastIgnoredTime === null,
        d && d.ignoredCount,
      );
      ...
      const s = lastEvent(n, "started");
      check(
        "X6b blocked idle starts still increment (scope: command redundancy only)",
        s &&
          s.ignored === true &&
          s.ignoredCount === 2 &&
          s.lastIgnoredTime !== null,
      ...
      const st = lastEvent(n, "stopped");
      check(
        "X6c redundant stop while disabled leaves blocked-start count intact",
        st && st.ignored === true && st.ignoredCount === 2,
        st && st.ignoredCount,
      );
      await n.close(false);
    }
...
__defineSuite("test-elapsed", async function () {
  ...
  await (async function main() {
    ...
    // -- Y1: paused elapsed freezes; resume unfreezes ---------------------------
    {
      ...
      const p = lastEvent(n, "paused");
      check(
        "Y1a paused event carries frozen elapsed ~3000ms",
        p && near(p.elapsedTime, 3000, 300),
        p && p.elapsedTime,
      );
      await sleep(1500);
      let q = query(n);
      check(
        "Y1b elapsed unchanged while paused (~3000, was: ~4500 and growing)",
        q && near(q.elapsedTime, 3000, 300),
        q && q.elapsedTime,
      );
      check(
        "Y1c frozen pair reconciles: elapsed + remaining = duration",
        q && near(q.elapsedTime + q.remainingTime, 10000, 300),
      ...
      q = query(n);
      check(
        "Y1d elapsed resumes growing (~4000ms)",
        q && near(q.elapsedTime, 4000, 400),
        q && q.elapsedTime,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    ...
    // -- Y2: idle queries report 0 ----------------------------------------------
    {
      ...
      let q = query(n);
      check(
        "Y2a query after stop -> elapsed 0 (was: growing forever)",
        q && q.elapsedTime === 0,
        q && q.elapsedTime,
      );
      n.receive({ payload: "go" });
      ...
      q = query(n);
      check(
        "Y2b query after expiry -> elapsed 0",
        q && q.elapsedTime === 0,
        q && q.elapsedTime,
      );
      await n.close(false);
    ...
    // -- Y3: genuine stopped/expired events carry the final elapsed --------------
    {
      ...
      const st = lastEvent(n, "stopped");
      check(
        "Y3a stopped event carries final elapsed ~3000ms",
        st && near(st.elapsedTime, 3000, 300),
        st && st.elapsedTime,
      );
      ...
      check(
        "Y3b expired event carries final elapsed ~2000ms",
        ex && near(ex.elapsedTime, 2000, 300),
        ex && ex.elapsedTime,
      );
      n.receive({ payload: "stop" }); // redundant (idle) - should carry 0
      const rst = lastEvent(n, "stopped");
      check(
        "Y3c redundant stop carries elapsed 0",
        rst && rst.ignored === true && rst.elapsedTime === 0,
        rst && rst.elapsedTime,
      );
      await n.close(false);
    }
    ...
    // -- Y4: stop while paused carries the frozen snapshot -----------------------
    {
      ...
      const st = lastEvent(n, "stopped");
      check(
        "Y4 stop-from-paused carries frozen elapsed ~2000ms (not ~3500)",
        st && near(st.elapsedTime, 2000, 300),
        st && st.elapsedTime,
      );
      await n.close(false);
    }
    ...
    // -- Y5: cooldown elapsed = time into the cooldown period --------------------
    {
      ...
      const q = query(n);
      check(
        "Y5a cooldown elapsed ~2000ms into the period (was: dead run's drift)",
        q && q.timerState === "cooldown" && near(q.elapsedTime, 2000, 400),
        q && q.elapsedTime,
      );
      check(
        "Y5b cooldown pair reconciles: elapsed + remaining ~= 6000ms",
        q && near(q.elapsedTime + q.remainingTime, 6000, 400),
        q && Math.round(q.elapsedTime + q.remainingTime),
      );
      n.receive({ payload: "stop" });
      const st = lastEvent(n, "stopped");
      check(
        "Y5c stop during cooldown carries cooldown-elapsed (~2000ms)",
        st && near(st.elapsedTime, 2000, 500),
        st && st.elapsedTime,
      );
      await n.close(false);
    }
    ...
    // -- Y6: threshold pause freezes; threshold stop carries final elapsed -------
    {
      ...
      const q = query(n);
      check(
        "Y6a threshold pause froze elapsed ~2000ms",
        q && q.timerState === "paused" && near(q.elapsedTime, 2000, 300),
        q && q.elapsedTime,
      );
      n.receive({ payload: "stop" });
      await n.close(false);

      const m = makeNode(RED, {
        donotresettimer: true,
      ...
      check(
        "Y6b threshold stop carries final elapsed ~2000ms",
        st && st.source === "internal" && near(st.elapsedTime, 2000, 300),
        st && st.elapsedTime,
      );
      await m.close(false);
    }
    ...
    // -- Y7: paused restore reconstructs the frozen elapsed ----------------------
    {
      ...
      check(
        "Y7a restored pause: elapsed reconciles with restored remaining",
        q &&
          q.timerState === "paused" &&
          near(q.elapsedTime + q.remainingTime, 10000, 300),
        q && Math.round(q.elapsedTime + q.remainingTime),
      );
      check(
        "Y7b restored pause: frozen elapsed survives downtime (~3000ms)",
        q && near(q.elapsedTime, 3000, 400),
        q && q.elapsedTime,
      );
...
__defineSuite("test-paused-restore", async function () {
  ...
  function persistFile(userDir, id) {
    return path.join(userDir, "timerevents-timers", id);
  }
  ...
  await (async function main() {
    ...
    // -- Z1: paused restore ignores downtime -----------------------------------
    {
      ...
      const q = query(n2);
      check(
        "Z1a remaining unchanged across downtime (~7000, was: ~5000)",
        q && q.timerState === "paused" && near(q.remainingTime, 7000, 300),
        q && Math.round(q.remainingTime),
      );
      check(
        "Z1b elapsed unchanged across downtime (~3000, was: ~5000)",
        q && near(q.elapsedTime, 3000, 300),
        q && Math.round(q.elapsedTime),
      );
      ...
      check(
        "Z1c resume after restore runs the full frozen ~7s",
        near(Date.now() - t0, 7000, 600),
        Date.now() - t0,
      );
      await n2.close(true);
    }
    ...
    // -- Z2: frozenElapsed independence survives (settime while paused) --------
    {
      ...
      const q = query(n2);
      check(
        "Z2a settime'd remaining restores exactly (9000)",
        q && q.remainingTime === 9000,
        q && q.remainingTime,
      );
      check(
        "Z2b frozen elapsed restores independently (~3000, not derived 1000)",
        q && near(q.elapsedTime, 3000, 300),
        q && Math.round(q.elapsedTime),
      );
      n2.receive({ payload: "stop" });
    ...
    // -- Z3: legacy persist file (no new fields) -> old fallback behavior ------
    {
      ...
      const q = query(n2);
      check(
        "Z3 legacy file falls back to target-minus-now (~5000 after 2s downtime)",
        q && q.timerState === "paused" && near(q.remainingTime, 5000, 400),
        q && Math.round(q.remainingTime),
      );
      n2.receive({ payload: "stop" });
      await n2.close(true);
    ...
    // -- Z4: regression - RUNNING restore still absorbs downtime ----------------
    {
      ...
      check(
        "Z4 running restore still deducts downtime (~5000 remaining)",
        s && near(s.remainingTime, 5000, 700),
        s && Math.round(s.remainingTime),
      );
      n2.receive({ payload: "stop" });
      await n2.close(true);
    }
...
__defineSuite("test-status-flicker", async function () {
  ...
  function isBlank(s) {
    return (
      !s || Object.keys(s).length === 0 || (!s.text && !s.fill && !s.shape)
    );
  }
  ...
  await (async function main() {
    const userDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "timerevents-flicker-"),
    );
    const RED = makeRED(userDir);
    require(path.join(__dirname, "..", "timer-events", "timer-events.js"))(RED);
    ...
    // -- F1: the flicker itself - query against a running timer ----------------
    {
      ...
      n.receive({ payload: "query" });
      check(
        "F1a query emits no blank status call (was: blank-then-repaint flicker)",
        n.statusCalls.every((s) => !isBlank(s)),
        JSON.stringify(n.statusCalls),
      );
      check(
        "F1b query still repaints a non-empty status",
        n.statusCalls.length >= 1 &&
          !isBlank(n.statusCalls[n.statusCalls.length - 1]),
    ...
    // handler, plus the gates, across running / paused / idle states.
    {
      ...
      for (const m of steps) {
        ...
        if (n.statusCalls.length === 0 || isBlank(last)) {
          ok = false;
          failedAt = JSON.stringify(m);
          break;
        }
      }
      check(
        "F2 all 26 command paths end with a non-empty status (invariant holds)",
        ok,
        failedAt,
      );
      await n.close(false);
    ...
    // -- F3: close handler still blanks (unrelated blank preserved) ------------
    {
      ...
      await n.close(false);
      check(
        "F3 node close still blanks the status (deliberate blank preserved)",
        n.statusCalls.length === 1 && isBlank(n.statusCalls[0]),
        JSON.stringify(n.statusCalls),
      );
    }
...
__defineSuite("test-fractional-label", async function () {
  ...
  await (async function main() {
    ...
    // -- R1: the reported bug - pause then resume, label must be whole seconds -
    {
      ...
      n.receive({ payload: "pause" });
      check(
        "R1a paused label has no fractional seconds",
        !hasFraction(lastStatusText(n)),
        lastStatusText(n),
      );
      n.receive({ payload: "resume" });
      check(
        "R1b resume label has no fractional seconds (the reported case)",
        !hasFraction(lastStatusText(n)),
        lastStatusText(n),
      );
      ...
      check(
        "R1c no subsequent countdown tick shows fractions",
        fractional.length === 0,
        JSON.stringify(fractional.map((s) => s.text)),
      );
      // Timing untouched: envelope still carries the exact frozen-derived ms
      n.receive({ payload: "query" });
      const q = lastEvent(n, "query");
      check(
        "R1d msg.remainingTime still exact ms (not rounded)",
        q && q.remainingTime % 1000 !== 0,
        q && q.remainingTime,
      );
      n.receive({ payload: "stop" });
      await n.close(false);
    ...
    // -- R2: sweep the other fractional-sync paths ------------------------------
    {
      ...
      check(
        "R2a adjusttime label whole seconds",
        !hasFraction(lastStatusText(n)),
        lastStatusText(n),
      );
      n.receive({ payload: "settime", settime: 1500 }); // the flagged settime edge: now rounds
      check(
        "R2b settime 1500 label rounds (edge resolved per display principle)",
        !hasFraction(lastStatusText(n)),
        lastStatusText(n),
      );
      n.receive({ payload: "poke1" });
      ...
      check(
        "R2c threshold Add Time label whole seconds",
        !hasFraction(lastStatusText(n)),
        lastStatusText(n),
      );
      n.receive({ payload: "stop" });
    ...
    // -- R4: timing precision unchanged - resume runs the exact frozen time -----
    {
      ...
      n.receive({ payload: "pause" });
      const frozen = (n.receive({ payload: "query" }), lastEvent(n, "query"))
        .remainingTime;
      n.receive({ payload: "resume" });
      const t0 = Date.now();
      await new Promise((res) => {
      ...
      check(
        "R4 resume-to-expiry matches the exact frozen ms (labels round; clocks don't)",
        near(ranFor, frozen, 150),
        ranFor + " vs frozen " + Math.round(frozen),
      );
      await n.close(false);
    }
